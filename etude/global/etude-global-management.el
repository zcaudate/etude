...
(use-package dashboard
  :init (progn (setq dashboard-startup-banner nil)
               (setq dashboard-items '((recents  . 15)
  		                                 (projects . 5)))
               (add-hook 'emacs-startup-hook 'etude/start-screen)))

(defun etude/neotree-toggle ()
  (interactive)
  (let ((cw (selected-window)))
    (neotree-toggle)
    (select-window cw)))

(defun etude/neotree-projectile-root ()
  (interactive)
...
(defun etude/neotree-projectile-locate ()
  (interactive)
  (save-excursion
    (let ((cw (selected-window)))
...
(provide 'etude-global-management)
