...
The previous code blocks are tangled together to produce a container setup script:
...
For containerisation of the setup, We will create a simple `Dockerfile` (Ubuntu bash) in the build directory.
