...
#+BEGIN_SRC bash :results output silent :cache no :tangle Ubuntu/setup.sh
...
~Mosh~
#+BEGIN_SRC bash :results output silent :cache no :eval no :tangle Ubuntu/mosh.sh
...
#+BEGIN_SRC docker-build :exports code :padline no :tangle Ubuntu/Dockerfile
FROM ubuntu
CMD bash
#+END_SRC
